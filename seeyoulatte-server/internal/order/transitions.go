...
var transitions []Transition = []Transition{
	Transition{
		...
		Guard: nil,
		Action: func(order *Order) error {
			order.State = string(constants.StatePaid)
			return nil
		},
	},
	Transition{},
...
func TransitionOrder(order *Order, targetState constants.OrderState, actor constants.Actor) error {
	for _, transition := range transitions {
		...
		if !fromCheck || !toCheck || !actorCheck {
			slog.Warn("No matching from and to state transition combinations in attempted transition.",
				"order_id", order.ID,
